...
function createPluginFromObject(newPlugin) {
  ...
  if (!newPlugin.onclick && !newPlugin.url)
    return {
  ...
  newPlugin = $.extend(
    {
      ...
      isDataType: function (inDataType) {
        return (
          this.dataType.toLowerCase().indexOf(inDataType.toLowerCase()) > -1
        );
      },
      dataTypes: function () {
      ...
      run: function (inVar, event) {
        ...
        data.page_description =
          ...
          "";
        data.page_url =
          (this.toolbar && this.toolbar.page_url) || location.toString();
        ...
        if (this.isDataType("text")) {
          data.text = inVar;
          data.dataType = "text";
          if (this.onclick) this.onclick(data);
          else if (this.url) this.createTabWithParams(this.url, inVar, data);
          ...
          if (Array.isArray(inVar) || inVar.jquery) inVar = inVar[0];
          ...
          if (this.onclick) {
            if (this.dataType == "image") data.dataType = "image";
            this.onclick(data);
          }
        } else {
          //no DataType
...
sb = {
  ...
  createTabWithParams: function (url, s, moreData) {
    ...
    newUrl = newUrl.replace(
      /{page_title}/g,
      encodeURIComponent(this.page_title)
    );
    newUrl = newUrl.replace(
      /{page_description}/g,
    ...
    newUrl = newUrl
      .replace(/%s/g, s)
      .replace("%c", encodeURIComponent(s.replace("img", "i3")))
      .replace(/%t/g, encodeURIComponent(moreData.title));
  ...
  dialog3: function (inElement) {
    ...
    var div = $(
      '<div style=width:inherit class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-front ui-dialog-buttons ui-draggable ui-resizable">' +
    ...
    div.innerHTML =
      '<link rel="stylesheet" href="jquery-ui-1.9.2.custom.min.css"></link><style>body {color:white;}</style>';
    iframe.contentDocument.body.appendChild(div);
    var askToClose = function () {
      iframe.remove();
    };
    var closeOnEsc = function (e) {
      if (e.keyCode == 27) askToClose();
  ...
  storageGet: function (inVar, def) {
    var key =
      (this.toolbar && this.toolbar.namespace) + "_" + this.key + "_" + inVar;
    var ans;
    if (extStorageGet) ans = extStorageGet(key);
    else ans = localStorage[key];
  ...
  storageSet: function (inVar, inVal) {
    var key =
      (this.toolbar && this.toolbar.namespace) + "_" + this.key + "_" + inVar;
    if (extStorageSet) return extStorageSet(key, inVal);
    else return (localStorage[key] = inVal);
  },
};
...
loadjQueryThings = function () {
  ...
  $.ajaxPrefilter(function (settings, orgSettings, xhr) {
    if (settings.showProgress) {
      var html = $(
        "<center class=progresscancel><div>Saving Online...<br><div style=height:20px;background-color:skyblue name=progressbar></div><button name=cancel>Cancel</button></div></center>"
      );
  ...
  function create$toolbar() {
    ...
    $.each(toolbar.obj_plugins, function (a, b, c) {
      ...
      if (options.theme) {
        ...
        dataMeta = [this.key, toolbar.icon_base];
        return dataMeta;
      } else {
        var style =
          (options.whiteIcons &&
      ...
      var img = $("img", html).on({
        ...
        mouseleave: function () {
          if (plugin.state != "down") $(this).removeClass("hover");
        },
  ...
  var updateMinMax = function () {
    if (toolbar.isMax()) {
      ...
      $(".tb_button", $toolbar)
        .parent()
        .hide()
        .slice(0, options.min_buttons_num)
        .show();
    }
  };
  this.isMax = function () {
  ...
  var init = function () {
    ...
    if (this.position == "static") {
      ...
      if (options.pushDown) {
        ...
        getFixedElements()
          .not($toolbar)
          .css("margin-top", function (i, value) {
...
function appendFirstStyle() {
  ...
  x +=
    ".ws_toolbar {z-index:100000} .ws_toolbar .ws_tb_btn {cursor:pointer;border:1px solid #555;padding:3px}   .tb_highlight{background-color:yellow} .tb_hide {visibility:hidden} .ws_toolbar img {padding:2px;margin:0px}";
