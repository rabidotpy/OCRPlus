...
function editor_obj() {
  ...
  function createCanvas() {
    $canvas = $(
      "<canvas class=tool style=position:relative></canvas>"
    ).appendTo($("#clipper")); //.data('clip-x1', clip.rx1).data('clip-y1', clip.ry1);
    if (true || addimg) {
    ...
    $canvas
      .add(newImg)
      .css({ "margin-left": -clip.rx1, "margin-top": -clip.ry1 });
  ...
  this.createLastCanvas = function (todo, moreParams) {
    ...
    if (imageChanged != localStorage["pngjpg"]) {
      ...
      for (var i = 0; i <= currentIndex; i++) {
        ...
        if (
          thisLevel.type != "crop" &&
          thisLevel.type != "delete" &&
          !thisLevel.hide
        )
          tool[thisLevel.type].draw({
            canvas: done,
            ctx: ctx,
            data: thisLevel,
          });
      ...
      if ($("#border1").hasClass("on")) addBorderToCanvas(done);
    ...
    if (todo == "save") {
      ...
      if (
        !!window.webkitIntent &&
        webkitIntent.action == "http://webintents.org/edit"
      ) {
        ...
        if (true) {
          ...
          createFile(function (filename) {
            $("#topText").html(
              '<a id=sss href="' + filename + '">Click here to Open</a>'
            );
    ...
    if (todo == "print") {
      imageChanged = false;
      $("#toPrintImage").one("load", function () {
        noMoreWait();
        if (canvasWidth < 700) {
  ...
  this.init = function () {
    ...
    (function () {
      ...
      $("anvas.tool").on("mousemove", function (e) {
        $this = $(this);
        ...
        myBorder = $(
          "<div class=myBorder style='position:relative;border:1px dashed 00eeff;z-index:1000;ursor:move'><div style=position:absolute;width:10;height:10;top:0;right:0;cursor:pointer>X</div></div>"
        ).appendTo("#divCanvasData");
        myBorder.css({
        ...
        $("div", myBorder).mouseenter(function () {
          tool.status = "moving";
        });
      });
    })();
    ...
    if (!!window.webkitIntent) {
      ...
      if (
        webkitIntent.action == "http://webintents.org/share" ||
        webkitIntent.action == "http://webintents.org/save"
      ) {
        $("#share").trigger("click");
      }
    }
  ...
  tool.text.finish = function () {
    ...
    a = {
      text: $("textarea").val(),
      font:
        $("textarea").css("font-size") + " " + $("textarea").css("font-family"),
    ...
    $("textarea").remove();
    tool[tool.current].draw(
      { canvas: canvas, ctx: c, data: currentLevel },
      "toolcreate"
    );
    tool.status = "ready";
  ...
  tool.text.draw = function (inX, toolcreate) {
    ...
    multiFillText(
      inX.canvas,
      inX.data.text,
      toolcreate ? 0 : inX.data.x - clip.rx1,
      toolcreate ? 0 : inX.data.y - clip.ry1,
      parseInt(lineHeight),
      parseInt(inX.data.width)
    );
    context.shadowOffsetX = 0;
  ...
  tool.crop.move = function (e) {
    ...
    y2 = e.y;
    if (
      $("#divCanvasData").scrollTop() + $("#divCanvasData").height() - e.y <
      35
    ) {
    ...
    if (
      $("#divCanvasData").scrollLeft() + $("#divCanvasData").width() - e.x <
      35
    ) {
    ...
    $("#crop_helper_left").css({
      ...
      top: 0,
      width: rx1 - clip.rx1,
      height: $canvasData.height() + scrollTop,
    });
    $("#crop_helper_top").css({
      "ackground-color": "red",
      left: rx1 - clip.rx1,
      top: 0,
      width: clip.rx2 - (rx1 - clip.rx1),
      height: ry1 - clip.ry1,
    });
  ...
  function pos(e) {
    ans = {
      x:
        clip.rx1 +
        e.pageX -
        $("#divCanvasData").offset().left +
        $("#divCanvasData").scrollLeft(),
      y:
        clip.ry1 +
        e.pageY -
        $("#divCanvasData").offset().top +
        $("#divCanvasData").scrollTop(),
    };
  ...
  tool.spray.move = function (e) {
    for (var j = c.lineWidth * 35; j > 0; j--) {
      var rndx =
        e.x + Math.round(Math.random() * (c.lineWidth * 15) - c.lineWidth * 2);
      var rndy =
        e.y + Math.round(Math.random() * (c.lineWidth * 15) - c.lineWidth * 2);
  ...
  tool.spray.draw = function (inX) {
    ...
    for (var i = 0; i < inX.data.points.length; i++) {
      thisPoint =
        parseInt(inX.data.points[i].y - inX.data.canvasOffset.y - 1) *
        inX.canvas.width *
        4;
    ...
    for (var i = 0; i < inX.data.points.length; i++) {
      thisPoint =
        parseInt(inX.data.points[i].y - inX.data.canvasOffset.y - 10) *
        inX.canvas.width *
        4;
      thisPoint +=
        parseInt(inX.data.points[i].x - inX.data.canvasOffset.x - 10) * 4;
  ...
  tool.free.draw = function (inX) {
    ...
    for (var i = 1; i < inX.data.points.length; i++) {
      inX.ctx.lineTo(
        inX.data.points[i].x - inX.data.canvasOffset.x,
        inX.data.points[i].y - inX.data.canvasOffset.y
      );
    }
  ...
  tool.line.draw = function (inX) {
    ...
    context.shadowOffsetY = 5;
    drawLine(
      inX.data.start.x - inX.data.canvasOffset.x,
      inX.data.start.y - inX.data.canvasOffset.y,
      inX.data.end.x - inX.data.canvasOffset.x,
      inX.data.end.y - inX.data.canvasOffset.y
    );
  ...
  tool.rectangle.draw = function (inX) {
    ...
    function drawRect(x1, y1, x2, y2, shadow) {
      ...
      if (inX.data.type == "full") inX.ctx.fill();
      else inX.ctx.stroke();
      inX.ctx.closePath();
    }
    context = inX.ctx;
  ...
  tool.star.begin = function (e) {
    ...
    currentLevel.start = {
      x: e.x - baseStarDim + 15,
      y: e.y - baseStarDim + 15,
    };
  ...
  tool.house.begin = function (e) {
    ...
    currentLevel.start = {
      x: e.x - baseHouseDim / 2,
      y: e.y - baseHouseDim / 2,
    };
  ...
  tool.house.draw = function (inX) {
    ...
    function drawhouse(x1, y1, x2, y2, shadow) {
      ...
      inX.ctx.globalAlpha = 1;
      inX.ctx.drawSvg(
        '<svg version="1.1" xmlns="http://www.w3.org/2000/svg"  x="0px" y="0px" width="200px" height="148.333px" viewBox="0 0 200 148.333" enable-background="new 0 0 200 148.333" xml:space="preserve"><polyline fill="none" stroke="#strokeColor#"  points="32.875,56.208 33.248,56.838 33.248,137.203 168.183,137.203 168.183,56.838 		"/> <polyline  fill="none" stroke="#strokeColor#" points="194.936,77.92 147.189,43.877 99.444,9.833 51.699,43.877 3.952,77.92 		"/>  </svg>'.replace(
          /#strokeColor#/g,
          inX.data.color
        ),
        x1,
  ...
  tool.arrow.draw = function (inX) {
    ...
    function drawArrow(x1, y1, x2, y2, shadow) {
      ...
      inX.ctx.lineTo(
        x2 - (DeltaY * dy) / L - (DeltaX * dx1) / L,
        y2 + (DeltaX * dy) / L - (DeltaY * dx1) / L
      );

      inX.ctx.lineTo(x2 + (DeltaX * dx2) / L, y2 + (DeltaY * dx2) / L);
  ...
  function changeTool(inTool) {
    ...
    } else localStorage["lastTool"] = inTool;
    stopTool();
  ...
  function onResize() {
    ...
    maxHeight =
      window.innerHeight - $("#header").height() - $("#fotter").height();
  ...
  function resetButtons() {
    ...
    $(".button[tag=undo]")[currentIndex > -1 ? "removeClass" : "addClass"](
      "disable"
    );

    $(".button[tag=redo]").attr(
      "status",
      maxIndex > currentIndex ? "on" : "Disable"
    );
    $(".button[tag=redo]")[
      maxIndex > currentIndex ? "removeClass" : "addClass"
    ]("disable");
        ...
        $(this)
          .attr({
            src:
              "images/drawing/" +
              $(this).attr("tag") +
              ($(this).attr("tag") == tool.current ? "On" : "Off") +
              ".png",
          })
          .css({ cursor: "pointer" });
        $(this)[
          $(this).attr("tag") == tool.current ? "addClass" : "removeClass"
        ]("on");
      } else
        $(this)
          .attr({
  ...
  function binds() {
    ...
    $(document.body).bind("mousemove", function (e) {
      ...
      if (dime) {
        ...
        if (tool.status == "started") {
          if ($.isFunction(tool[tool.current].move))
            tool[tool.current].move(dime);
        }
      }
    });
    ...
    $(document.body).bind("mouseup", function (e) {
      ...
      if (tool.current) tryFunc(tool[tool.current].up);
    });
      ...
      hr = $.ajax({
        ...
        complete: function (a, b, c) {
          ...
          if (
            ...
            $("#topText").html(
              "Please try again in some minutes. We are working to solve this issue."
            );
          } else {
            if (cancel) return;
  ...
  function save() {
    if (localStorage["lastTool"] == "text") changeTool("line");
  ...
  function pleaseWait() {
    ...
    $("#serviceButtons button")
      .attr("disabled", "true")
      .css({ cursor: "default", border: "none" });
  ...
  start = function () {
    ...
    googleAuth.authorize(function () {
      ...
      xhr.setRequestHeader(
        "Authorization",
        "OAuth " + googleAuth.getAccessToken()
      );
      var meta = {
        title: screenshot.title,
        mimeType: "image/png",
        description:
          "Taken by Screenshot Extension. https://www.openscreenshot.com",
      };
      ...
      xhr.onload = function (e) {
        ...
        if (e.error) {
          ...
          xh.open(
            "POST",
            "https://www.googleapis.com/urlshortener/v1/url?key=AIzaSyC5S6Uxzx6oUxIEHMWZU4eOm7mL6DupChw"
          );
          xh.setRequestHeader(
            "Authorization",
            "OAuth " + googleAuth.getAccessToken()
...
$.fn.sethover = function (x) {
  ...
  function sethover(elm) {
    ...
    showHover = function () {
      ...
      arrow = $(
        "<div class=sss style=position:absolute;background-repeat:no-repeat;width:16px;height:16px;background-image:url(old/nd/ui_top_triangle.png)></div>"
      );
...
function addBorderToCanvas(e) {
  ...
  for (
    var i = e.width - corner.width - down.width;
    i > down.width;
    i -= down.width
  )
    c.drawImage(down, i, e.height - down.height);
  for (; i >= 0; i--) c.drawImage(empty, i, e.height - down.height);
  for (
    var i = e.height - corner.width - right.height;
    i > right.height;
    i -= right.height
  )
    c.drawImage(right, e.width - right.width, i);
  for (; i >= 0; i--) c.drawImage(empty, e.width - empty.width, i);
}
...
if (location.hash == "#paste") {
  ...
  $(function () {
    $(
      '<center class=paste_modal style="position:absolute;top:150px;width:100%"><br><span style="padding:20px;font-size:40px;border:1px solid black">Press Ctrl+V to paste an image</span></center>'
